...
std::string ofxPhysical::getCurveName(AnimCurve c){
...
void ofxPhysical::drawCurve(int x, int y, int size){
	...
	float s = size;
	float steps = size;
	string name = ofxPhysical::getCurveName(curveStyle_);
	glPointSize(1);
...
void ofxPhysical::fillInParams(float &p1, float &p2, float &p3){
...
float ofxPhysical::calcCurveAt( float percent ){
...
float ofxPhysical::calcCurveAt(float percent, AnimCurve type, float param1, float param2, float param3){
...
void ofxPhysical::update(float dt){
...
void ofxPhysical::startAnimation(){
	direction_ = 1;
	percentDone_ = 0.0f;
	delay_ = 0.0f;
	animating_ = true;
...
void ofxPhysical::startAnimationAfterDelay(float delay){
	direction_ = 1;
	delay_ = delay;
	waitTime_ = delay_;
	//animating_ = false;
...
void ofxPhysical::setDuration( float seconds ){
	transitionSpeed_ = 1.0f / seconds;
}


void ofxPhysical::setRepeatType( AnimRepeat repeat ){
	repeat_ = repeat;
}


void ofxPhysical::setCurve( AnimCurve curveStyle){
	curveStyle_ = curveStyle;
}


float ofxPhysical::getPercentDone(){ 
	return percentDone_; 
}		

void ofxPhysical::setPercentDone(float p){
	if( p < 0.0f ) p = 0.0f;
	percentDone_ = p;
}

bool ofxPhysical::isAnimating(){ 
	return animating_;
}


bool ofxPhysical::hasFinishedAnimating(){ 
	return !animating_;
}

bool ofxPhysical::isWaitingForAnimationToStart(){ 
	return ( delay_ > 0.0f );
}

float ofxPhysical::getCurrentSpeed(){
	float r = fabs( direction_ * currentSpeed_ / (lastDT_ * transitionSpeed_));
	return r;
}

bool ofxPhysical::isOrWillBeAnimating(){ 
	return isAnimating() || isWaitingForAnimationToStart() ;
}

void ofxPhysical::pause(){
	paused_ = true;
}


void ofxPhysical::resume(){
	paused_ = false;
}
